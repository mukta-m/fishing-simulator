...
int addFish(Pond *p, unsigned char size, char *species){
  ...
  // else, place fish at the end of array
    ...
    printf("type of fish %s\n", p->fish[p->numFish]->species);
    printf("current num of fish: %d\n", p->numFish);
    ++ p->numFish;
    return 1;
  }
}
...
void giveAwayFish(Fisher *fisher1, Fisher *fisher2, Pond *p){
  ...
  if (ableToTake == 0){
    for (int k = 0; k < size1; k++){
      p->fish[pondSize] = fisher1->bucket[k];
      ++pondSize;
      ++p->numFish;
      --fisher1->numFish;
    }
    ...
    for (int i = 0; i < ableToTake; i ++){
      if (size1 > 0){
        fisher2->bucket[size2] = fisher1->bucket[i];
        ++fisher2->numFish;
        ++size2;
        --fisher1->numFish;
        --size1;
    ...
    if (size1 != 0){
      for (int j = 0; j < size1; j++){
        p->fish[pondSize] = fisher1->bucket[j];
        ++pondSize;
        ++p->numFish;
        --fisher1->numFish;
      }
...
// use for loop to free all the fish in ponds and buckets
  for (int i = 0; i < mainPtr->numFish; i++){
    free(mainPtr->fish[i]);
  }

  for (int j = 0; j < fredPtr->numFish; j++){
    free(fredPtr->bucket[j]);
  }

  for (int k = 0; k < suzyPtr->numFish; k++){
    free(suzyPtr->bucket[k]);
  }
